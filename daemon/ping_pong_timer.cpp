 ...
 *
 * Once in a while, each daemon sends PING messages to some other daemons
 * (those with a smaller IP address).
 */
 ...
 * other daemons. It gets initialized with that amount of time as defined in
 ...
 */
ping_pong_timer::ping_pong_timer(prinbeed * p, std::int64_t interval_us)
    : timer(interval_us)
    , f_prinbeed(p)
{
