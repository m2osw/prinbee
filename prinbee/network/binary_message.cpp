...
constexpr prinbee::struct_description_t g_context_description[] =
{
    prinbee::define_description(
          prinbee::FieldName(g_system_field_name_magic)
        , prinbee::FieldType(struct_type_t::STRUCT_TYPE_MAGIC)
...
{
    auto const it(std::find_if(f_list.begin(), f_list.end(), [name](auto const & item)
        {
...
{
    auto const it(std::find_if(f_list.begin(), f_list.end(), [id](auto const & item)
        {
...
bool binary_message::deserialize_context_message(msg_context_t & context)
{
    ...
    context.f_schema_version = description->get_uinteger("schema_version");
