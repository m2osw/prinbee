...
public:
    ...
    Num(size_t n, word w, bool sign = false): words(n, w), neg(sign){}

    Num(const word *a, const word *b, bool sign = false) : words(a, b), neg(sign){}
    ...
    Num(const char *c, word base = 10, char **endptr = NULL): neg(false){
        ...
        if (endptr) *endptr = const_cast<char*>(c);
    }
